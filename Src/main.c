...
// the following is previous code used for testing
...
void softi2c_send_ack(GPIO_TypeDef * scl_port, int scl_pin, GPIO_TypeDef * sda_port, int sda_pin) {
	softi2c_line_mode(scl_port, scl_pin, 0);
	softi2c_delay();
	softi2c_line_mode(sda_port, sda_pin, 0);
	softi2c_delay();
...
int softi2c_write8(GPIO_TypeDef * scl_port, int scl_pin, GPIO_TypeDef * sda_port, int sda_pin, int device_addr, int data) {
	int nack = 0;
	softi2c_sig_start(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, device_addr << 1 | 0);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
...
int softi2c_write8_3(GPIO_TypeDef * scl_port, int scl_pin, GPIO_TypeDef * sda_port, int sda_pin, int device_addr, int data0, int data1, int data2) {
	int nack = 0;
	softi2c_sig_start(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, device_addr << 1 | 0);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, data0);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, data1);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, data2);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_sig_stop(scl_port, scl_pin, sda_port, sda_pin);
	return nack;
}
...
int softi2c_read_auto16(GPIO_TypeDef * scl_port, int scl_pin, GPIO_TypeDef * sda_port, int sda_pin, int device_addr) { // this is for tmp275 idk whats going on
	int nack = 0;
	softi2c_sig_start(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send8(scl_port, scl_pin, sda_port, sda_pin, device_addr << 1 | 1);
	nack += softi2c_read_nack(scl_port, scl_pin, sda_port, sda_pin);
	int data = softi2c_read8(scl_port, scl_pin, sda_port, sda_pin) << 8;
	softi2c_send_ack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_sig_repeated_start(scl_port, scl_pin, sda_port, sda_pin);
	data |= softi2c_read8(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_send_ack(scl_port, scl_pin, sda_port, sda_pin);
	softi2c_sig_stop(scl_port, scl_pin, sda_port, sda_pin);
	return data;
}
...
void op2_printb32(uint32_t value) {
	for (int i = 31; i >= 0; i--) {
		int bit = 0;
		if (value & (1 << i)) {
			bit = 1;
		...
		uart_tx_char(OP2_SCI_UART, bit + 0x30);
	}
}
...
int main() {
	...
	init_lpuart(LPUART1, GPIOG, 7, 8, GPIOG, 8, 8);

	gpio_mode(GPIOE, 2, 1, 0, 0, 0); // D1
	gpio_mode(GPIOD, 3, 1, 0, 0, 0); // D2
	gpio_mode(GPIOD, 1, 1, 0, 0, 0); // D3
	gpio_mode(GPIOD, 14, 1, 0, 0, 0); // D4
	gpio_mode(GPIOG, 11, 1, 0, 0, 0); // D5

	gpio_set(GPIOE, 2, 0); // D1
	gpio_set(GPIOD, 3, 0); // D2
	gpio_set(GPIOD, 1, 1); // D3
	gpio_set(GPIOD, 14, 1); // D4
	gpio_set(GPIOG, 11, 0); // D5

	while (0) { // opr3 factor code v1
		delay_ms(1000);
		op2_printstr(".");
		delay_ms(1000);
		op2_printstr(".");
		...
		op2_printstr(".");
		delay_ms(1000);
		gpio_set(GPIOD, 3, 1); // D2
		op2_printnewline();
		op2_printstr("# Orbital Platform R3");
		op2_printnewline();
		op2_printstr("# Integrated flight systems for REALOP-1 mission");
		op2_printnewline();
		op2_printstr("#        Space and Satellite Systems at UC Davis");
		op2_printnewline();
	//	op2_printstr("# Serial U7L3A01BE78JC");
	//	op2_printstr("# Serial U7L3A02B9UP7D");
	//	op2_printstr("# Serial U7L3A03BN8RGQ");
		op2_printstr("# Serial U7L3A04BY2Q3M");
	//	op2_printstr("# Serial U7L3A05BD91R4");
		op2_printnewline();
		op2_printstr("# [mcu0] ");
		...
		while (1) {
			delay_ms(400);
			gpio_set(GPIOE, 2, 1); // D1
			delay_ms(100);
			gpio_set(GPIOE, 2, 0); // D1
			delay_ms(400);
			gpio_set(GPIOG, 11, 1); // D5
			delay_ms(100);
			gpio_set(GPIOG, 11, 0); // D5
		}
	}
...
// 	op2_printstr("orbital platform r3. mcu serial bytes: ");
// 	op2_printuint32(*(uint32_t*)(UID_BASE));
// 	op2_printstr(", ");
// 	op2_printuint32(*(uint32_t*)(UID_BASE+0x4));
// 	op2_printstr(", ");
...
#define tmp0_I2C GPIOE, 4, GPIOE, 3 // tmp0: PE4 scl, PE3 sda
#define tmp0_ADDR 74
#define pwrmon_I2C GPIOA, 15, GPIOD, 0 // tmp0: PA15 scl, PD0 sda

	init_softi2c(tmp0_I2C);
	init_softi2c(pwrmon_I2C);

	for (int i = 0; i <= 0xFF; i++) {
		op2_printstr("addr ");
		op2_printint16(i);
		if (softi2c_probe(tmp0_I2C, i)) {
			op2_printstr(" ack!");
		}
		op2_printnewline();
	}
	// ok tmp0 addr is 74 as expected

	softi2c_write_reg(tmp0_I2C, tmp0_ADDR, 0x01, 0x60);
	delay_ms(10);
	softi2c_write8(tmp0_I2C, tmp0_ADDR, 0x00);

	while (1) {
		int temp = softi2c_read_auto16(tmp0_I2C, tmp0_ADDR)>>4;
		op2_printb32(systick_time);
		op2_printstr(", ");
		op2_printint16(temp);
		op2_printstr(", ");
		op2_printint16(temp/16);
		op2_printstr(" C, ");
	...
	while(1) {
		...
		gpio_set(GPIOD, 1, 1); // D3
		delay_ms(100);
		gpio_set(GPIOD, 1, 0); // D3
		gpio_set(GPIOD, 14, 1); // D4
		delay_ms(100);
		gpio_set(GPIOD, 14, 0); // D4
		gpio_set(GPIOG, 11, 1); // D5
		delay_ms(100);
		gpio_set(GPIOG, 11, 0); // D5
	}
	...
	while(1); // 2024 10 10 opr3. below is opr2 and 1 code
	...
	//	while(1);
		...
		while (1) {
			...
			op2_printstr(",");
			op2_printint16(op2_imu_read_gyro_z());
			op2_printstr(",");
			op2_printuint32(th);
			op2_printnewline();
