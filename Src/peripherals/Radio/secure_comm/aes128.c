...
/**
 * Encrypts a maximum length message of 256 bytes by the AES-128 CBC method of operation
 * based on the length specified. The CBC method "requires" the message to be
 * padded with the PKCS#7 scheme and therefore changes the length of the message.
 *
...
void Encrypt_Array(uint8_t* PlainText, size_t* n) {
  ...
  AES_CBC_encrypt_buffer(&StructofAES, PlainText, *n+remainder);
