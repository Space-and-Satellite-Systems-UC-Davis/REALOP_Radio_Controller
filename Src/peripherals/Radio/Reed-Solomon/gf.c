...
uint8_t RSGF_multiply_terms(uint16_t x, uint16_t y){
    if (x == 0 || y == 0)
        return 0;
    return RSGF_exp[(RSGF_log[x] + RSGF_log[y]) % 255];
}
...
uint8_t RSGF_divide_terms(uint8_t x, uint8_t y){
    if(x == 0) return 0;
    return RSGF_exp[(RSGF_log[x] + 255 - RSGF_log[y]) % 255];
}
...
uint8_t RSGF_power(uint8_t x, intmax_t power){
    intmax_t i = RSGF_log[x];
    i *= power;
    i %= 255;
    if(i < 0) i = i + 255;
    return RSGF_exp[i % 255];
}
...
uint8_t RSGF_inverse(uint8_t x){
    return RSGF_exp[(255 - RSGF_log[x]) % 255]; /* == RSGF_divide_terms(1, x); */
}
...
RSGF_scale_polynomial(const struct RSGF_Polynomial *p, struct RSGF_Polynomial *newp, uint16_t x) {
    ...
    for(uint16_t i = 0; i < p->length; i++){
        RSGF_retreive_pointer(newp)[i] = RSGF_multiply_terms(RSGF_retreive_pointer(p)[i], x);
    }
}
...
RSGF_add_polynomials(const struct RSGF_Polynomial *p, const struct RSGF_Polynomial *q, struct RSGF_Polynomial *newp) {
    ...
    memset(RSGF_retreive_pointer(newp), 0, newp->length * sizeof(uint8_t));

    for(uint8_t i = 0; i < p->length; i++){
        RSGF_retreive_pointer(newp)[i + newp->length - p->length] = RSGF_retreive_pointer(p)[i];
    }

    for(uint8_t i = 0; i < q->length; i++){
        RSGF_retreive_pointer(newp)[i + newp->length - q->length] ^= RSGF_retreive_pointer(q)[i];
    }
}
...
RSGF_multiply_polynomials(const struct RSGF_Polynomial *p, const struct RSGF_Polynomial *q, struct RSGF_Polynomial *newp) {
    ...
    // below was "memset(RSGF_retreive_pointer(newp), 0, newp->length * sizeof(uint8_t));"
    RSGF_reset_polynomial(newp);
    /* Compute the polynomial multiplication (just like the outer product of two vectors,
     * we multiply each coefficients of p with all coefficients of q) */
    for(uint8_t j = 0; j < q->length; j++){
        for(uint8_t i = 0; i < p->length; i++){
            RSGF_retreive_pointer(newp)[i+j] ^= RSGF_multiply_terms(RSGF_retreive_pointer(p)[i], RSGF_retreive_pointer(q)[j]); /* == r[i + j] = gf_add(r[i+j], gf_mul(p[i], q[j])) */
        }
    }
}
...
RSGF_divide_polynomials(const struct RSGF_Polynomial *p, const struct RSGF_Polynomial *q, struct RSGF_Polynomial *newp) {
    if(RSGF_retreive_pointer(p) != RSGF_retreive_pointer(newp)) {
        memcpy(RSGF_retreive_pointer(newp), RSGF_retreive_pointer(p), p->length*sizeof(uint8_t));
    }
    ...
    for(int i = 0; i < (p->length-(q->length-1)); i++){
        coef = RSGF_retreive_pointer(newp)[i];
        if(coef != 0){
            for(uint8_t j = 1; j < q->length; j++){
                if(RSGF_retreive_pointer(q)[j] != 0)
                    RSGF_retreive_pointer(newp)[i+j] ^= RSGF_multiply_terms(RSGF_retreive_pointer(q)[j], coef);
            }
        }
    }

    size_t sep = p->length-(q->length-1);
    memmove(RSGF_retreive_pointer(newp), RSGF_retreive_pointer(newp)+sep, (newp->length-sep) * sizeof(uint8_t));
...
RSGF_evaluate_polynomial(const struct RSGF_Polynomial *p, uint16_t x) {
    uint8_t y = RSGF_retreive_pointer(p)[0];
    for(uint8_t i = 1; i < p->length; i++){
        y = RSGF_multiply_terms(y, x) ^ RSGF_retreive_pointer(p)[i];
    }
    return y;
}